...
const Login = () => {
    ...
    return (
        <section className="login-page h-screen flex justify-center items-center">
            <Pending />
            <form onSubmit={handleSubmit(onLogin)} className='flex flex-col justify-center text-center text-white w-[340px] gap-2'>
                ...
                {error !== null && (
                    <p className='text-red-500 text-center text-xs'>
                        {error?.message?.includes('401') ? 'Password yang anda masukkan salah' : 'Akun Tidak Terdaftar'}
                    </p>
                )}
                <Input {...register('identifier')} type="text" required placeholder="Username Atau Email" className='p-[22px] border-gray-900 focus:border-zinc-700 bg-zinc-900/80 rounded-xl' />
